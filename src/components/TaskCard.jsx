...
const TaskCard = ({ task, fetchAllTasks }) => {
  const handleCompleteTask = async () => {
    try {
      await axios.patch(`https://express-intro-starting-point-git-patch-franks-projects-601765f0.vercel.app/${task.id}`, {
  ...
  const handleDeleteTask = async () => {
    try {
      await axios.delete(`https://express-intro-starting-point-git-patch-franks-projects-601765f0.vercel.app/${task.id}`);
      fetchAllTasks();
    } catch (error) {
